...
def execute_e(
    ...
    wts = time.time()
    start: float = process_time()
    foo(fn_vr, a1, a2, a3, a7, fn, rd, fd, pc, a, b, c, d, e, time_array[1:-1], dt)
...
def foo(fn_vr, a1, a2, a3, a7, fn, rd, fd, pc, a, b, c, d, e, maxt, dt):
    ...
    for t in maxt:
        ...
        for j, f in enumerate(fn_vr):
            a7[j][0][i], a7[j][1][i], a7[j][2][i], a7[j][3][i], a7[j][3][i] = fn_vr[j](
                i, a1[j], a2[j], a3[j]
            )
...
@njit
def update_virtual_reservoirs(fn_vr, a1, a2, a3, a4, a5, a6, a7, i):
    for j, f in enumerate(fn_vr):

        fn_vr[j](i, a7[j], a1[j], a2[j], a3[j], a4[j], a5[j], a6[j])
...
def build_vr_list(lor: list) -> tuple:
    ...
    fn = numba.typed.List.empty_list(
        types.UniTuple(types.float64, 5)(
            ...
            #types.ListType(types.float64),       # 3
        ).as_type()
    )
