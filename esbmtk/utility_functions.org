...
def create_bulk_connections(ct: dict, M: any, mt: int = "1:1") -> None:
    ...
    # make sure we have enough entries. Otherwise apply to each connnection
    c_ct = ct.copy()
    for k, v in c_ct.items():  # loop over connections
        if isinstance(k, tuple):
            nk = len(k)
        else:
            nk = 1
        print(f"nk = {nk}")  # get number of connections
        for k1, p in v.items():  # loop over connection properties
            # properties are a list, make sure the list can have multiple
            # values if this is a group connection
            if isinstance(p, list):
                print(f"len (p) = {len(p)}, p = {p}")
                assert len(p) >= nk
            else:  # if shorter, fill properties
                l = []
                for i in range(0, nk):
                    l.append(p)
                ct[k][k1] = l
    ...
    for k, v in ct.items():
        if isinstance(k, tuple):
            ...
            for c in k:
                create_connection(c, v, M, nk)
        elif isinstance(k, str):
            create_connection(k, v, M, nk)
        else:
...
def create_connection(n: str, p: dict, M: any, nk: int) -> None:
