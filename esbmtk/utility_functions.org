...
def sum_lists(r_list, f_list, dir_list, v_list, r0_list, i, dt):
    ...
    for j in range(r_steps):
        ...
        for u in range(f_steps):
            ...
            mass += f_list[j][u][0][i] * dir_list[j][u]  # mass
            li += f_list[j][u][1][i] * dir_list[j][u]  # li
...
** COMMENT Build List plain style
#+BEGIN_SRC ipython :tangle utility_functions.py
...
def build_lists(lor):
    ...
    for r in lor:  # loop over all reservoirs
        ...
        tf: list = []  # temp list for flux data
        td: list = []  # temp list for direction data

        # loop over all fluxes
        for f in r.lof:
            fd_list =[f.m, f.l, f.h, f.d]
            tf.append(fd_list)
...
** COMMENT Build Lists Numba style
...
def build_lists(lor):
    ...
    for r in lor:  # loop over all reservoirs
        fd_list = List()
        for e in [r.m, r.l, r.h, r.d, r.c]:
            fd_list.append(List(e))

        r_list.append(fd_list)
        v_list.append(float(r.volume))
        r0_list.append(float(r.sp.r))
