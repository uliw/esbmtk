...
class Model(esbmtkBase):
    ...
    def save_data(self, **kwargs) -> None:
        ...
        for r in self.lor:
            r.__write_data__(start, stop, stride)
...
class Reservoir(esbmtkBase):
    ...
    def __write_data__(self, start :int, stop :int, stride :int) -> None:
        ...
        df[f"{self.n} {sn} [{smu}]"] = self.m[::stride]  # mass
        df[f"{self.n} {sn} [{cmu}]"] = self.c[::stride]  # concentration
        df[f"{self.n} {sp.ln}"] = self.l[::stride]  # light isotope
        df[f"{self.n} {sp.hn} "] = self.h[::stride]  # heavy isotope
        df[f"{self.n} {sdn} {sds}"] = self.d[::stride]  # delta value

        for f in self.lof:  # Assemble the headers and data for the reservoir fluxes
            df[f"{f.n} {sn} [{fmu}]"] = f.m[::stride]
            df[f"{f.n} {sn} [{sp.ln}]"] = f.l[::stride]
            df[f"{f.n} {sn} [{sp.hn}]"] = f.h[::stride]
            df[f"{f.n} {sn} {sdn} {sds}"] = f.d[::stride]
