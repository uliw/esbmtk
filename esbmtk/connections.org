...
#+BEGIN_SRC ipython
class Equilibrium(RateConstant):
    ...
    def __misc__init(self):
        ...
        [Rl, Cl] = sort_list_arguments(self.left, [Reservoir, Number], em)
        [Rr, Cr] = sort_list_arguments(self.right, [Reservoir, Number], em)
    ...
    def __call__(self, reservoir: Reservoir, i: int) -> None:
        ...
        # loop over Rl and extract concentrations
        rl = np.array(1, 1, 1, 1)
        rr = np.array(1, 1, 1, 1)

        # calculate the product for left side
        for rl in self.Rl:
            rl *= r[i - 1]

        # calculate the product for right side
        for rr in self.Rr:
            rr *= r[i - 1]

        get- scale factor
        scale = self.lk * np.prod(rl) - np.prod(rr) * self.rk
