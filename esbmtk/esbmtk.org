...
class Reservoir(esbmtkBase):
    ...
    def __read_state__(self)->None:
        ...
        print(f"number of headers {len(header_list)}")
        ...
        for n in header_list:
            name = n.split(" ")[0]
            if name == self.name:
              col =  self.__assign__data__(name,df,col,True)
            elif is_name_in_list(name, self.lof):
              col =  self.__assign__data__(name,df,col,False)  
            else:
                print(f"No '{name}' in {self.n}\n")
                raise ValueError("Unable to find Reservoir of Flux Name")
            

    def __assign__data__(self, name :str, df: pd.DataFrame, col :int, res :bool)->int:
        """
        ...
        print(f"header = {name}, col={col}")
        self.m[0:3] = df.iloc[0:3,col].to_numpy()
        col += 1
        self.l[0:3] = df.iloc[0:3,col].to_numpy()
        col += 1
        self.h[0:3] = df.iloc[0:3,col].to_numpy()
        col += 1
        self.d[0:3] = df.iloc[0:3,col].to_numpy()
        col += 1

        if res: # if type is reservoir
             self.c[0:3] = df.iloc[0:3,col].to_numpy()
             col += 1
