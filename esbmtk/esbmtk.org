    ...
    def execute(new: [NDArray, Float], time: [NDArray, Float],
        ...
        for t in time[0:-1]:  # loop over the time vector except the first
            ...
            for r in lor:  # loop over all reservoirs
                ...
                # do sum of fluxes in this reservoir
                for f in flux_list:
                    direction = r.lio[f.n]
                    new[0] = new[0] + f.m[i] * direction
                    new[1] = new[1] + f.l[i] * direction
                    new[2] = new[2] + f.h[i] * direction

                #new = array([ms, ls, hs])
                new = new * r.mo.dt  # multiply with timestep
