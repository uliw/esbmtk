...
class esbmtkBase(object):
    ...
    def __register_name__(self) -> None:
        ...
        # this sub object.
        if self.register: # This is part of a group
            if self in self.register.lmo:
                raise NameError(f"{self.name} is a duplicate in {self.register}. Please fix")
            setattr(self.register, self.name, self)
            self.register.lmo.append(self)

        else: # this is a regular registration
            if type(self) == Model: # Model only exist in the global NS
...
class Reservoir(esbmtkBase):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.lkk: Dict[str, any] = {
            ...
            "transform": str,
            "plot": str,
            "register": (SourceGroup,SinkGroup,ReservoirGroup,ConnectionGroup),
        }
