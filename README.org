...
* News
  ...
  - 0.8.0.0 Cleanup of naming scheme which is now strictly
    hierarchical. I.e., species definitions are no longer part of the
    Model (i.e., =M.DIC=), rather you have to write
    =M.Carbon.DIC). This will break model code, but is easy to fix
