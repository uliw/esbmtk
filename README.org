...
and in jupyter notebook format)
  ...
  - A simple carbon cycle model which shows how to set up the model,
    and read an external csv file to force the model
  - A simple carbon cycle model which demonstrates how to a simple
    signal, and how to use the rate constant process to adjust
    concentration dependent flux rates
