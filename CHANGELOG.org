...
- Unit-registered output variables: append _u to reservoir attributes
  (e.g., M.S_b.c_u, M.S_b.m_u, M.time_u) to retrieve values with units
  attached.
...
:CUSTOM_ID: bug-fixes
:END:
- Fix reverse_time plot display for figures with multiple x-axes (plots
...
:PROPERTIES:
:CUSTOM_ID: internal-developer-changes
:END:
...
- Added =src/esbmtk/initialize_unit_registry.py= to centralize Q_/ureg.
...
:PROPERTIES:
:CUSTOM_ID: breaking-changes-migration-notes
:END:
...
- Public API reshuffle:
  ...
  - Unit registry: Q_ and ureg are now provided by
    =esbmtk.initialize_unit_registry= (available via from esbmtk import
    Q_, ureg). If you previously created your own UnitRegistry, adopt
