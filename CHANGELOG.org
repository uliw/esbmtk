* Changelog
- 0.14.2.1 ESBMTK now requires python 3.12. Cleaned up repository structure. This may result in import errors. Please get in touch if it does. Reworked many class definitions to improve readability and documentation. Fixed a regression that caused the model code to abort when weathering and carbonate calculations are used in conjunction with isotope calculations.  Added =debug= keyword to the Model object. 
