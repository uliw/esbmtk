...
class Model(esbmtkBase):
    ...
    def run(self) -> None:
        ...
        start:float = process_time()  # this has nothing todo with self.time below!
...
class RateConstant(Process):
    ...
    def __init__(self, **kwargs: Dict[str, any]) -> None:
        ...
        # get default names and update list for this Process
        self.__defaultnames__()  # default kwargs names
        self.lkk.update({"slope": Number, "C0": Number}) # update the allowed keywords
        self.lrk.extend(["reservoir", "slope", "C0"])  # new required keywords
        # self.lod dict with default values if none provided
        self.__initerrormessages__() 
        self.bem.update({"slope": "a number", "reservoir": Reservoir, "C0": "a number"})
        self.__validateandregister__(kwargs)  # initialize keyword values
        self.__postinit__()  # do some housekeeping
