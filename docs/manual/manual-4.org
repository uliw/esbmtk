...
Notes: 
- The flux splitting scheme employed in =carbonate_system_4= currently assumes that the carbonate compensation depth (i.e. =z_{cc}=) is always in the deep box.
...
| species	| basins	| piston_velocity |
|CO2	|A_sb, I_sb, P_sb, H_sb	| 4.8 m/d|
| O2	|A_sb, I_sb, P_sb, H_sb	| 4.8 m/d|
