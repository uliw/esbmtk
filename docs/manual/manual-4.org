...
=carbonate_system_4= is an extension of CS2 and CS3, and enables carbonate chemistry for models with three vertical ocean layers, i.e., models with a surface layer, an intermediate layer, and a deep layer (in addition to a sediment layer/burial box). =carbonate_system_4= splits Boudreau's carbonate dissolution fluxes and adds them to either the intermediate box or the deep box depending on the relative depth of =zsat= (i.e., saturation depth) in each timestep. The flux-splitting scheme therefore consists of two scenarios: either =zsat= is deeper than =zint=, or =zint= is deeper than =zsat=. 

Both scenarios are visualized below:
...
In the first scenario (=zint= deeper than =zsat=), BNS, i.e., the CaCO3 dissolution between =z0= and =zsat=, is now split into two by =zint=, one of which is added to the intermediate box, and the other to the deep box. BDS, BCC, and BPDC remain unmodified (i.e., are computed in the same manner as CS2). In the second scenario ((=zsat= deeper than =zint=), the computation of BNS is unmodified, and the whole of it is added to the intermediate box. However, both the undersaturation and respiration components of BDS (i.e., dissolution of CaCO3 raining down between zsat and zcc) are split into two, and added accordingly to either the intermediate or the deep box. BCC and BPDC again remain unmodified and are computed in the same manner as they are in CS2.

=carbonate_system_4= may be added to a model as follows:
...
M.B_b.DIC.lif.append(f) #appends the db2_export flux object to the sink.lif list
#+END_SRC
...
- There is no need to add a =ConnectionProperties= instance to handle PIC flux connections between the Intermediate and Deep boxes, or the Deep and Burial boxes. This is handled entirely by CS4. As in CS3, however, one must declare a =ConnectionProperties= instance (with the bypass "bp" keyword) to remove the total carbonate export fluxe from =source box=.
