...
Provided that the model has terms for DIC and TA, pH calculations for a given @@rst::py:class:`esbmtk.extended_classes.Reservoir()`@@ instance are added using the @@rst::py:func:`esbmtk.carbonate_chemistry.add_carbonate_system_1()`@@ function:
#+BEGIN_SRC ipython
...
The following figure provides an overview of the parametrizations and variables used by the  @@rst::py:func:`esbmtk.carbonate_chemistry.carbonate_system_2()`@@ and @@rst::py:func:`esbmtk.carbonate_chemistry.add_carbonate_system_2()`@@ functions.
#+attr_org: :width 600
#+attr_rst: :width 800
#+attr_latex: :width 0.8\textwidth
#+name: boudreau
#+caption:  Overview of the parametrizations and variables used by the 
#+caption: @@rst::py:func:`esbmtk.carbonate_chemistry.carbonate_system_2()`@@ and
#+caption:  @@rst::py:func:`esbmtk.carbonate_chemistry.add_carbonate_system_2()`@@ functions.
...
Notes:
 ...
 - The @@rst::py:func:`esbmtk.carbonate_chemistry.carbonate_system_2()`@@ function only returns [H^{+}] and the dissolution flux for the given box. It does not return the burial flux.
...
*NOTE (November 2025): this is currently in the early stages of development.*

Carbonate system 4 is an extension of CS2 and CS3, and enables carbonate chemistry for models with three vertical ocean layers, i.e., models with a surface layer, an intermediate layer, and a deep layer (in addition to a sediment layer/burial box). 

The =carbonate_system_4= function splits Boudreau (2010)'s carbonate dissolution fluxes and adds them to either the intermediate box or the deep box depending on the relative depth of z_{sat} (i.e., saturation depth) computed in each timestep. The flux-splitting scheme therefore consists of two scenarios: either z_{sat} is deeper than z_{int}, or z_{int} is deeper than z_{sat}. Both scenarios are visualized below:

#+attr_org: :width 600
#+attr_rst: :width 800
#+attr_latex: :width 0.8\textwidth
#+name: CS4 flux-splitting scheme 1
#+caption:  Overview of the modifications made by CS4 for the scenario where =zsat= is deeper than =zint=.
#+caption: Image Credit: Atlas Changulani
[[./CS4fluxsplittingscheme1.png]]

#+attr_org: :width 600
#+attr_rst: :width 800
#+attr_latex: :width 0.8\textwidth
#+name: CS4 flux-splitting scheme 1
#+caption:  Overview of the modifications made by CS4 for the scenario where =zint= is deeper than =zsat=.
#+caption: Image Credit: Atlas Changulani
[[./CS4fluxsplittingscheme2.png]]

In the first scenario (z_{sat} deeper than z_{int}), B_{NS} (the CaCO_{3} dissolution between z_{0} and z_{sat}) is now split into two by z_{int}. One of these split components is added to the intermediate box, and the other to the deep box. B_{DS}, B_{CC}, and B_{PDC} remain unmodified and are computed in the same manner as they are in CS2. 

In the second scenario (z_{int} deeper than z_{sat}), the computation of B_{NS} is unmodified, and the whole of it is added to the intermediate box. However, both the undersaturation and respiration components of B_{DS} (dissolution of CaCO_{3} raining down between z_{sat} and z_{cc}) are split into two by z_{int}, and added accordingly to either the intermediate or the deep box. B_{CC} and B_{PDC} again remain unmodified and are computed in the same manner as they are in CS2.
...
f = M.flux_summary(filter_by="burial", return_list=True) #"burial" is the flux from D_b to B_b 
M.B_b.DIC.lif.append(f) #appends the "burial" flux object to the sink.lif list
#+END_SRC

Notes: 
- The flux splitting scheme employed in =carbonate_system_4= currently assumes that the carbonate compensation depth (i.e. =zcc=) is always in the deep box.
...
A more generalized version of Carbonate System 2/3/4 capable of handling n vertical ocean layers is currently in development. 

** Gas Exchange
ESBMTK implements gas exchange across the Air-Sea interface as a @@rst::py:class:`esbmtk.connections.Species2Species()`@@ instance, between a @@rst::py:class:`esbmtk.extended_classes.GasReservoir()`@@ and a @@rst::py:class:`esbmtk.base_classes.Species()`@@ instance. In the following example, we first declare a =Gasreservoir= and then connect it with a regular surface box. Note that the CO_{2} gas transfer calculation requires that the respective surface reservoir carries the =CO2aq= tracer as calculated by the @@rst::py:func:`esbmtk.carbonate_chemistry_carbonate_system_1.()`@@ function since the gas-transfer depends on the dissolved CO_{2} rather than on the DIC concentration.
#+BEGIN_SRC ipython
GasReservoir(
    name="CO2_At",
    species=M.CO2,
...
#+END_SRC
Defining gas transfer for O_2  uses the same approach.  Currently ESBMTK provides useful defaults for CO_{2,} O_2 and N_2 only. See the isotope chapter on how to define gas exchange reactions for custom species.
