...
In the first step, one needs to define a model object that describes fundamental model parameters. The following code first loads the following ESBMTK classes that will help with model construction:
 - @@rst::py:class:`model.Model()`@@
 - @@rst::py:class:`base_classes.Reservoir()`@@
 - @@rst::py:class:`esbmtk.connections.ConnectionProperties()`@@ class
 - @@rst::py:class:`base_classes.SourceProperties()`@@ class
 - @@rst::py:class:`base_classes.SinkProperties()`@@ class
...
#+END_SRC
Next we use the @@rst::py:class:`model.Model()`@@  class to create a model instance that defines basic model properties. Note that units are automatically translated into model units. While convenient, there are some important caveats: 
...
@@rst::py:meth:`model.Model.flux_summary()`@@ method of the model instance:
#+BEGIN_SRC ipython
...
The model time axis is available as =M.time= and the model supports the @@rst::py:class:`model.Model.connection_summary()`@@ and @@rst::py:class:`model.Model.flux_summary()`@@   
