...
#+caption: Speciess contain the data for a given species. Multiple
...
#+caption: reservoir groups (and/or Speciess) are defined by a connection object that defines e.g., 
#+caption: a rate function etc. Connection objects can be dynamically modified
#+caption: by a Signal. Reservoirs have various sub-classes that provide access to e.g., 
