...
Each row in the excel table represents one reservoir definition. Reservoir geometry, temperature, pressure, and salinity, initial species concentrations, and isotope concentrations can be specified as columns, as shown in the example table below:

|name | type | z_top | z_bottom | area_percentage | temperature | pressure | salinity | DIC | TA  |
|-----+------+------+----------+-----------------+-------------+----------+----------+-----+-------+----+----|
| A_sb | reservoir| 0| -100	| 0.26	| 20	| 5	| 34.7	| 2210	| 2310 |
| A_ib |reservoir |-100	|-1000	|0.26	|10	|100	|34.7	|2210	|2310| 
| A_db|reservoir|-1000	|-6000	|0.26	|2	|240	|34.7	|2210	|2310|	
| Fw | source| | | | | | | | | 
| Fb	| sink| | | | | | | | | 
...
Notes:
- The function only accepts reservoir geometry information provided via the =z_top=, =z_bottom=, and =area_percentage= parameters at this moment. 
...
- All columns named =delta_[species]= are interpreted as isotope values of [species].

*** Creating transport matrices from Excel

The function =create_transport_matrix_from_excel= creates transport connections between reservoirs from a worksheet. Each row defines a single connection. Required excel columns are =source=, =sink=, =flux_id=, and =sc=. 
...
The scaling factor =sc= be specified in Sverdrups (e.g. “10 Sverdrup”) or as a variable defined within the python file.

Note that mixing fluxes are symmetric, so for each mix_up flux, a corresponding “mix_down” flux need not be specified and will be created automatically.
...
It is recommended to check out [example] in [[https://github.com/uliw/ESBMTK-Examples/tree/main][ESBMTK-Examples]] for a more detailed overview of the functions described above.
** Model forcing
